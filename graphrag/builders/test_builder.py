...
def build_test_edges(graph_store: GraphStore, test_file_path: str) -> Dict:
    """Create TEST -> CRU edges."""
    try:
        with open(test_file_path, 'r') as f:
            data = json.load(f)
    except Exception as e:
        print(f"[EDGE ERROR] Failed to load {test_file_path}: {e}")
        return {"error": str(e)}
    ...
    for test in all_tests:
        ...
        if (test_id and req_id and 
            graph_store.node_exists(test_id) and 
            graph_store.node_exists(req_id)):
    ...
    print(f"[EDGE SUMMARY] Created {edges_created} TEST->CRU edges")
    return {"edges_created": edges_created, "tests_processed": len(all_tests)}